...
async function fetchPageContent(pageData) {
  ...
  const response = await fetch(
    `${pageData.baseURL}/pages/${pageData.slug}/content/`
  );
...
function setupCarousels() {
  ...
  for (let i = 0; i < carouselElements.length; i++) {
    ...
    const itemsContainer = carousel.getElementsByClassName(
    ...
    const determineSideCutoffs = () => {
      ...
      if (
        Math.ceil(itemsContainer.scrollLeft) + itemsContainer.clientWidth <
    ...
    const determineSideCutoffsRateLimited = throttledDebounce(
  ...
  for (let i = 0; i < searchWidgets.length; i++) {
    ...
    const handleKeyDown = (event) => {
      ...
      if (event.key == "Enter") {
        ...
        const url = searchUrlTemplate.replace(
          "!QUERY!",
...
function setupGroups() {
  ...
  for (let g = 0; g < groups.length; g++) {
    ...
    const tabs = group.getElementsByClassName("widget-group-contents")[0]
      .children;
    let current = 0;
...
function setupCollapsibleLists() {
  const collapsibleLists = document.querySelectorAll(
...
function setupCollapsibleGrids() {
  const collapsibleGridElements = document.querySelectorAll(
  ...
  for (let i = 0; i < collapsibleGridElements.length; i++) {
    ...
    const getCardsPerRow = () => {
      return parseInt(
        getComputedStyle(gridElement).getPropertyValue("--cards-per-row")
      );
    };
...
function timeInZone(now, zone) {
  ...
  const diffInMinutes = Math.round(
    (timeInZone.getTime() - now.getTime()) / 1000 / 60
  );
...
function zoneDiffText(diffInMinutes) {
  ...
  if (minutes == 0) {
    return {
      text: `${sign}${hours}h`,
      title: `${hours} hour${hourSuffix} ${signText}`,
    };
  }
  ...
  if (hours == 0) {
    return {
      text: `${sign}${minutes}m`,
      title: `${minutes} minutes ${signText}`,
    };
  }

  return {
    text: `${sign}${hours}h~`,
    title: `${hours} hour${hourSuffix} and ${minutes} minutes ${signText}`,
  };
}
...
function setupClocks() {
  ...
  for (var i = 0; i < clocks.length; i++) {
    ...
    const localWeekdayElement =
      localTimeContainer.querySelector("[data-weekday]");
    ...
    for (var z = 0; z < timeZoneContainers.length; z++) {
      ...
      updateCallbacks.push((now) => {
        const { time, diffInMinutes } = timeInZone(
          now,
          timeZoneContainer.dataset.timeInZone
        );
        setZoneTime(time);
  ...
  const updateClocks = () => {
    ...
    for (var i = 0; i < updateCallbacks.length; i++) updateCallbacks[i](now);
...
async function setupCalendars() {
  ...
  for (let i = 0; i < elems.length; i++) calendar.default(elems[i]);
}
...
function initThemePicker() {
  ...
  if (themeChoicesInHeader) {
    themeChoicesInHeader.replaceWith(themeChoicesInMobileNav.cloneNode(true));
  }

  const presetElems = findAll(".theme-choices .theme-preset");
  const themePreviewElems = document.getElementsByClassName(
    ...
    presetElement.addEventListener("click", () => {
      ...
      changeTheme(themeKey, () => {
        ...
        Array.from(themePreviewElems).forEach((preview) => {
          preview
            .querySelector(".theme-preset")
            .replaceWith(presetElement.cloneNode(true));
        });
