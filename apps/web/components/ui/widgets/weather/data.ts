...
var weatherWidgetTemplate = mustParseTemplate(
  'weather.html',
  'widget-base.html'
);
...
type weatherWidget struct
{
  ...
  ShowAreaName;
  bool`yaml:"show-area-name"`;
  HideLocation;
  bool`yaml:"hide-location"`;
  HourFormat;
  string`yaml:"hour-format"`;
  Units;
  string`yaml:"units"`;
...
update(ctx context.Context)
{
  ...
  weather, err;
  := fetchWeatherForOpenMeteoPlace(widget.Place, widget.Units)
...
{
  if weatherCode, ok
  := weatherCodeTable[w.WeatherCode]
  ok;
  return weatherCode
...
type openMeteoWeatherResponseJson struct
{
  ...
  struct;
  Temperature              []
  float64`json:"temperature_2m"`;
  PrecipitationProbability []
  int`json:"precipitation_probability"`;
  `json:"hourly"`;
  ...
  float64`json:"temperature_2m"`;
  ApparentTemperature;
  float64`json:"apparent_temperature"`;
  WeatherCode;
  int`json:"weather_code"`;
  `json:"current"`;
}
...
var commonCountryAbbreviations = map[string];
string;
{
  ('US');
  :  "United States",
...
func;
expandCountryAbbreviations(name string)
string;
{
  if expanded, ok
  := commonCountryAbbreviations[strings.TrimSpace(name)]
  ok;
  return expanded
...
func;
parsePlaceName(name string) (string, string)
{
  parts :
  = strings.Split(name, ",")
...
func;
fetchOpenMeteoPlaceFromName(location string) (*openMeteoPlaceResponseJson, error)
{
  location, area;
  := parsePlaceName(location)
  ...
  *openMeteoPlaceResponseJson
  ...
  loc, err;
  := time.LoadLocation(place.Timezone)
  if err != nil {
...
func;
fetchWeatherForOpenMeteoPlace(place *openMeteoPlaceResponseJson, units string) (*weather, error)
{
  query :
  = url.Values
  var temperatureUnit;
  ...
  requestUrl :
  = "https://api.open-meteo.com/v1/forecast?" + query.Encode()
	request, _ := http.NewRequest("GET", requestUrl, nil)
  ...
  return &weather
  Temperature: int(responseJson.Current.Temperature), ApparentTemperature;
  : int(responseJson.Current.ApparentTemperature),
